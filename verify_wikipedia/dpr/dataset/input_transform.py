...
WaferPassage = collections.namedtuple(
    "WaferPassage",
    [
        "text",
        "title",
...
class WaferPreprocessor(SchemaPreprocessor):
    def preprocess_query(self, query, training: bool = False):
        ...
        if not hasattr(self.cfg, "input_transform") or (
            self.cfg.input_transform.question.type is None or len(self.cfg.input_transform.question.type) == 0
        ):
        ...
        for k in field_tensors.keys():
            if fields[k] is not None:
                field_tensors[k] = self.tensorizer.text_to_tensor(
                    fields[k], apply_max_len=False, add_special_tokens=False
                )
        ...
        assert (
            sum([len(t) for t in tensor_list]) == self.tensorizer.max_length
        ), f"{[len(t) for t in tensor_list]} {sum([len(t) for t in tensor_list])}!={self.tensorizer.max_length}"
