...
def wikitext_to_claims(wikitext):
    ...
    for section in mw.parse(wikitext).get_sections(flat=True, include_lead=True):
        ...
        for n in section.nodes:
            ...
            if nc == 'Heading':
                ...
                # e.g., Section::::Reputation.:Rankings.
                section_heading = 'Section:::'
                for i in range(0, lvl_idx + 1):
                    section_heading += f':{section_headings[i]}.'
            elif nc == 'List':
                in_list = True
                list_text = 'BULLET::::- '
            elif in_list:
                ...
                url_sections.append(section_heading or 'Section::::Abstract.')
            else:
    ...
    for i, cit in enumerate(re.finditer(r'\[CIT\]', article_text)):
        pre_cit = article_text[:cit.start()].replace('[CIT]', '')
        pre_cit_words = pre_cit.split()
        if len(pre_cit_words) > 150:
            pre_cit = ' '.join(pre_cit_words[-150:])
        after_cit = article_text[cit.end():].replace('[CIT]', '')
        after_cit_words = after_cit.split()
        if len(after_cit_words) > 50:
            after_cit = ' '.join(after_cit_words[:50])
        yield urls[i], url_sections[i], f'{pre_cit}[CIT] {after_cit}'
