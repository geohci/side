...
from dpr.models.biencoder import DocumentBiEncoder
from dpr.models.ranker_loss import RankerLoss, RerankerNllLoss
...
class BERTRerankerCrossEncoder(DocumentBiEncoder):
    ...
    def prepare_model_inputs(self, batch: dict, dataset):
        ...
        cols = torch.arange(1, c_len, device=question_ids.device).view(1, -1).repeat(ctx_b_size, 1) + (
            (all_cross_ids != dataset.tensorizer.get_pad_id()).sum(-1).view(-1, 1) - 1
        )
